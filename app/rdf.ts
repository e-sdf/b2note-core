...
function anRecord2RdfObj(an: AnRecord) {
  return {
    "rdf:RDF": {
      ...
      "rdf:Description": mkDescItems(an)
    }
  };
}

export function mkRDF(anRecord: AnRecord): string {
  const doc = anRecord2RdfObj(anRecord);
  return xml.create(doc).end({ pretty: true });
}